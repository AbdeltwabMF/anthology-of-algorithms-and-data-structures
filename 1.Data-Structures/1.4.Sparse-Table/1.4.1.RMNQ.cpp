  ...
  public :
    ...
    void build()
    {
        ...
        for(int j = 1, k, d; j <= _LOG; ++j) // the two nested loops below have overall time complexity = O(n log n)
        {
            ...
            {
                if(_A[ST[i][j - 1]] <= _A[ST[i + d][j - 1]])
                    ST[i][j] = ST[i][j - 1];     // starting subarray at index = i     with length = d
                else
                    ST[i][j] = ST[i + d][j - 1]; // starting subarray at index = i + d with length = d
            }
        }
    }
    ...
    int query(int l, int r) // this query is O(1)
    {
        ...
        if(_A[ST[l][lg]] <= _A[ST[l + range - k][lg]])
            return ST[l][lg];
        else
