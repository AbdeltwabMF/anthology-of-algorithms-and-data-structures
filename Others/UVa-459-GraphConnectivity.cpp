...
int Head[26 + 10], Next[26 * 25 / 2 + 10], To[26 * 25 / 2 + 10], ne, CCs;
bool vis[36];
string input, mxEgde;
...
void Solve()
{
    ...
    while(getline(cin, input) && sz(input) == 2)
    {
        addEdge((int)input[0] - 64, (int)input[1] - 64);
        addEdge((int)input[1] - 64, (int)input[0] - 64);    
    }

    for (int i = 1; i <= mxEgde[0] - 64; ++i) if(!vis[i])
    {
        ++CCs; DFS(i); 
    }
