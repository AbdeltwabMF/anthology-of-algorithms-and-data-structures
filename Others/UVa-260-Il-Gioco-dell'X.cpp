...
bool valid(int r, int c, char color)
{
    return r >= 0 && r < n && c >= 0 && c < n && maze[r][c] == color;
}

bool FloodFill(int r, int c, char color)
{
    if(!valid(r, c, color)) return false;
    if(color == 'b' && r == n - 1) return true;
    if(color == 'w' && c == n - 1) return true;
    ...
    for (int i = 0; i < 6; ++i)
    {
        ok |= FloodFill(r + dr[i], c + dc[i], color);
    }
    return ok;
}
...
void Solve()
{
    ...
    for (int i = 0; i < n; ++i)
        black |= FloodFill(0, i, 'b');

    for (int i = 0; i < n; ++i)
        white |= FloodFill(i, 0, 'w');
