...
string ar[55], in_str, _;
int n, cnt;
char seenCnt[500], mostSeen;

bool valid(int r, int c, char newColor)
{
    return r >= 0 && r < n && c >= 0 && c < sz(ar[r]) && ar[r][c] == ' ';
}

void FloodFill(int r, int c, char newColor)
{
    if(!valid(r, c, newColor)) return;

    ar[r][c] = newColor;

    for(int i = 0; i < 4; ++i)
        FloodFill(r + dx4[i], c + dy4[i], newColor);
}
...
void Solve()
{
    ...
    for(int j = 0; j < sz(ar[i]); ++j)
        if(ar[i][j] != ' ')
        {
            if(++seenCnt[(int)ar[i][j]] > cnt)
                cnt = seenCnt[(int)ar[i][j]], mostSeen = ar[i][j];
        }

    for(int i = 0; i < n; ++i)
    for(int j = 0; j < sz(ar[i]); ++j)
        if(ar[i][j] != mostSeen && ar[i][j] != ' ')
        {
            char tmp = ar[i][j];
            ar[i][j] = ' ';
...
void MultiTest(bool Tests)
{
    while(getline(cin, in_str))
    {
        if(in_str[0] == '_')
        {
            _ = in_str; memset(seenCnt, 0, sizeof(seenCnt)); cnt = 0;
            Solve();
            n = 0; continue;
        }
        ar[n++] = in_str;
